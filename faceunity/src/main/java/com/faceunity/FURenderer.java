...
public class FURenderer implements OnFUControlListener {
    ...
    private volatile  float mFilterLevel = 1.0f;//滤镜强度
    private volatile  float mSkinDetect = 1.0f;//肤色检测开关
    private volatile  float mHeavyBlur = 0.0f;//重度磨皮开关
    private volatile  float mBlurLevel = 0.7f;//磨皮程度
    private volatile  float mColorLevel = 0.3f;//美白
    private volatile  float mRedLevel = 0.3f;//红润
    private volatile  float mEyeBright = 0.0f;//亮眼
    private volatile  float mToothWhiten = 0.0f;//美牙
    private volatile  float mFaceShape = BeautificationParams.FACE_SHAPE_CUSTOM;//脸型
    private volatile  float mFaceShapeLevel = 1.0f;//程度
    private volatile  float mCheekThinning = 0f;//瘦脸
    private volatile  float mCheekV = 0.5f;//V脸
    private volatile  float mCheekNarrow = 0f;//窄脸
    private volatile  float mCheekSmall = 0f;//小脸
    private volatile  float mEyeEnlarging = 0.4f;//大眼
    private volatile  float mIntensityChin = 0.3f;//下巴
    private volatile  float mIntensityForehead = 0.3f;//额头
    private volatile  float mIntensityMouth = 0.4f;//嘴形
    private volatile  float mIntensityNose = 0.5f;//瘦鼻
    private volatile  float mChangeFrames = 0f;//渐变帧数
    // 默认滤镜，粉嫩效果
    ...
    public void config(RenderConfig renderConfig) {
        sFilterName = renderConfig.mFilterName;
        mFilterLevel = renderConfig.mFilterLevel;
        mSkinDetect = renderConfig.mSkinDetect;
        mHeavyBlur = renderConfig.mHeavyBlur;
        mBlurLevel = renderConfig.mBlurLevel;
        mColorLevel = renderConfig.mColorLevel;
        mRedLevel = renderConfig.mRedLevel;
        mEyeBright = renderConfig.mEyeBright;
        mToothWhiten = renderConfig.mToothWhiten;
        mFaceShape = renderConfig.mFaceShape;
        mFaceShapeLevel = renderConfig.mFaceShapeLevel;
        mCheekThinning = renderConfig.mCheekThinning;
        mCheekV = renderConfig.mCheekV;
        mCheekNarrow = renderConfig.mCheekNarrow;
        mCheekSmall = renderConfig.mCheekSmall;
        mEyeEnlarging = renderConfig.mEyeEnlarging;
        mIntensityChin = renderConfig.mIntensityChin;
        mIntensityForehead = renderConfig.mIntensityForehead;
        mIntensityMouth = renderConfig.mIntensityMouth;
        mIntensityNose = renderConfig.mIntensityNose;
        mChangeFrames = renderConfig.mChangeFrames;
    }

    public RenderConfig generateConfig() {
        RenderConfig renderConfig = new RenderConfig();
        renderConfig.mFilterName = sFilterName;
        renderConfig.mFilterLevel = mFilterLevel;
        renderConfig.mSkinDetect = mSkinDetect;
        renderConfig.mHeavyBlur = mHeavyBlur;
        renderConfig.mBlurLevel = mBlurLevel;
        renderConfig.mColorLevel = mColorLevel;
        renderConfig.mRedLevel = mRedLevel;
        renderConfig.mEyeBright = mEyeBright;
        renderConfig.mToothWhiten = mToothWhiten;
        renderConfig.mFaceShape = mFaceShape;
        renderConfig.mFaceShapeLevel = mFaceShapeLevel;
        renderConfig.mCheekThinning = mCheekThinning;
        renderConfig.mCheekV = mCheekV;
        renderConfig.mCheekNarrow = mCheekNarrow;
        renderConfig.mCheekSmall = mCheekSmall;
        renderConfig.mEyeEnlarging = mEyeEnlarging;
        renderConfig.mIntensityChin = mIntensityChin;
        renderConfig.mIntensityForehead = mIntensityForehead;
        renderConfig.mIntensityMouth = mIntensityMouth;
        renderConfig.mIntensityNose = mIntensityNose;
        renderConfig.mChangeFrames = mChangeFrames;
        return renderConfig;
    }
